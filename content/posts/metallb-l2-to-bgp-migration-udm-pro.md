...
`holdTime` is accepted, `keepaliveTime` isn't. No real loss, because MetalLB derives keepalive as `holdTime/3`, and BGP negotiates the lower of the two hold times anyway, so the router's value governs. Native mode also has no BFD, which makes hold time your only failure detection.
...
Client-side apply, which is kubectl's default and ArgoCD's, merges the port list on `port` **alone**. Two entries with port 8555 collide on that key and the second is silently discarded. The `last-applied-configuration` annotation kept listing the UDP port, which made it look like it had been applied.
...
The obvious zero-downtime move, putting both old and new addresses on one Service, doesn't work:
...
For the stated goal of stopping MetalLB handing out infrastructure addresses, the pool narrowing alone would have done it. BGP was the bigger swing, and it bought:
